#!/usr/bin/env python3
"""
Script to finetune Octo-small with V-JEPA2 encoder on CAST dataset.

This script:
1. Loads Octo-small pretrained weights
2. Replaces vision encoder with V-JEPA2
...
Usage:
    ...
    torchrun --nproc_per_node 1 scripts/finetune_cast_vjepa.py --debug
...
FLAGS = flags.FLAGS
flags.DEFINE_string("config", "scripts/configs/finetune_cast_vjepa_config.py", "Path to config file")
flags.DEFINE_string("pretrained", "hf://rail-berkeley/octo-small-1.5", "Pretrained checkpoint path")
flags.DEFINE_string("name", "cast_vjepa_finetune", "Experiment name for logging")
...
def main(_):
    # ── Load config ───────────────────────────────────────────────────────
    logging.info(f"Loading config from {FLAGS.config}")
    ...
    logging.info("=" * 80)
    logging.info("FINETUNING OCTO-SMALL WITH V-JEPA2 ON CAST DATASET")
    logging.info("=" * 80)
    ...
    # ── Step 2: Override with V-JEPA2 / CAST configuration ────────────────
    logging.info("\n🔧 Configuring V-JEPA2 encoder...")
    ...
    meta['config']['model']['num_tokens_dict'] = {
        'primary': 256,   # 16×16 patches from V-JEPA2 at 256x256 resolution
    ...
    logging.debug("   Skipping: V-JEPA2 encoder (has own pretrained weights)")
    ...
    logging.debug("\n❄️  Freezing V-JEPA2 and T5 text encoder weights...")
    ...
    if distributed_state.is_main_process:
        if FLAGS.save_dir is not None:
            save_dir = Path(FLAGS.save_dir) / FLAGS.name
            save_dir.mkdir(parents=True, exist_ok=True)
            logging.info(f"\n💾 Checkpoints will be saved to: {save_dir}")
            # Save config
            config_save_path = save_dir / "config.py"
            import shutil
            shutil.copy(FLAGS.config, config_save_path)
            logging.info(f"   Saved config to: {config_save_path}")
        else:
            logging.warning("\n⚠️  No save_dir specified, checkpoints will NOT be saved!")
    ...
    def run_validation(step: int):
        ...
        for val_i, val_batch in enumerate(val_dataloader):
            ...
            # Sample actions for first batch visualization
            if val_i == 0:
            ...
            # Viz only first batch
            if val_i == 0:
                gt_actions = val_batch["action"]
                mse = torch.mean((pred_actions - gt_actions) ** 2).item()
                val_mses.append(mse)

                if distributed_state.is_main_process:
                    for s_idx in range(min(4, val_batch["action"].shape[0])):
                        viz_img = _make_sample_viz(val_batch, pred_actions, s_idx)
                        viz_images.append(viz_img)

        avg_val_loss = np.mean(val_losses)
        avg_val_mse = np.mean(val_mses) if val_mses else 0.0
        ...
        if distributed_state.is_main_process:
            ...
            if val_mses:
                log_dict["val/mse"] = avg_val_mse
            if viz_images:
                log_dict["val/samples"] = viz_images
            wandb.log(log_dict, step=step)
            logging.info(f"   Validation → loss={avg_val_loss:.4f}" + 
                        (f", mse={avg_val_mse:.6f}" if val_mses else ""))
    ...
    for i, batch in tqdm.tqdm(
        ...
        if distributed_state.is_main_process and (i + 1) % FLAGS.log_interval == 0:
            ...
            logging.info(
                f"Step {i:5d} | loss={loss.item():.4f} | "
                f"lr={current_lr:.2e} | grad_norm={grad_norm.item() if isinstance(grad_norm, torch.Tensor) else grad_norm:.3f}"
            )
